"use client";
// /properties — browse all properties with filter sidebar, keyword search
// and sorting. Filters are stored in URL search params so they're shareable.

import { useQuery } from "@tanstack/react-query";
import { api } from "@/lib/axios";
import type { ApiResponse, Property, Category } from "@/types";
import { PropertyCard, PropertyCardSkeleton } from "@/components/PropertyCard";
import { useSearchParams, useRouter, usePathname } from "next/navigation";
import { Suspense, useEffect, useState } from "react";
import { Search, SlidersHorizontal, X, ArrowUpDown } from "lucide-react";

// ── Data fetchers ─────────────────────────────────────────────────────────────
async function fetchProperties(params: URLSearchParams) {
  const { data } = await api.get<ApiResponse<Property[]>>(
    `/api/properties?${params.toString()}`
  );
  return data;
}

async function fetchCategories() {
  const { data } = await api.get<ApiResponse<Category[]>>("/api/categories");
  return data.data;
}

function PropertiesContent() {
  const searchParams = useSearchParams();
  const pathname = usePathname();
  const router = useRouter();

  // Keyword search input (debounced before it hits the URL)
  const [keyword, setKeyword] = useState(searchParams.get("search") ?? "");

  // Helper to update a single filter param while keeping others
  function setParam(key: string, value: string) {
    const params = new URLSearchParams(searchParams.toString());
    if (value) {
      params.set(key, value);
    } else {
      params.delete(key);
    }
    // Reset to page 1 when filters change
    params.delete("page");
    router.push(`${pathname}?${params.toString()}`);
  }

  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect
    setKeyword(searchParams.get("search") ?? "");
  }, [searchParams]);

  useEffect(() => {
    const timer = setTimeout(() => {
      if (keyword !== (searchParams.get("search") ?? "")) {
        setParam("search", keyword);
      }
    }, 400);
    return () => clearTimeout(timer);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [keyword]);

  function clearFilters() {
    setKeyword("");
    router.push(pathname);
  }

  const { data, isLoading } = useQuery({
    queryKey: ["properties", searchParams.toString()],
    queryFn: () => fetchProperties(searchParams),
  });

  const { data: categories } = useQuery({
    queryKey: ["categories"],
    queryFn: fetchCategories,
    staleTime: Infinity,
  });

  const properties = data?.data ?? [];
  const meta = data?.meta;
  const hasFilters =
    searchParams.has("city") ||
    searchParams.has("categoryId") ||
    searchParams.has("minRent") ||
    searchParams.has("maxRent") ||
    searchParams.has("bedrooms") ||
    searchParams.has("search");

  // Sort options map to backend sortBy/sortOrder params
  const sortOptions = [
    { value: "", label: "Newest First" },
    { value: "rentAmount-asc", label: "Price: Low to High" },
    { value: "rentAmount-desc", label: "Price: High to Low" },
    { value: "bedrooms-desc", label: "Most Bedrooms" },
  ];
  const sortValue =
    `${searchParams.get("sortBy") ?? ""}-${searchParams.get("sortOrder") ?? ""}`;
  const activeSort = sortOptions.find((o) => o.value === sortValue)?.value ?? "";
  const currentPage = Number(searchParams.get("page") ?? 1);

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-text-primary">
          Browse <span className="gradient-text">Properties</span>
        </h1>
        <p className="text-text-secondary text-sm mt-1">
          {meta ? `${meta.total} listings found` : "Searching…"}
        </p>
      </div>

      {/* ── Keyword search bar ─────────────────────────────────────────── */}
      <div className="relative max-w-2xl mb-8">
        <Search size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted" />
        <input
          type="text"
          value={keyword}
          onChange={(e) => setKeyword(e.target.value)}
          placeholder="Search by title, description or location…"
          className="form-input pl-11"
          id="keyword-search"
        />
        {keyword && (
          <button
            onClick={() => {
              setKeyword("");
              setParam("search", "");
            }}
            aria-label="Clear search"
            className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-text-primary"
          >
            <X size={16} />
          </button>
        )}
      </div>

      <div className="flex flex-col lg:flex-row gap-6">
        {/* ── Filter Sidebar ────────────────────────────────────────────── */}
        <aside className="lg:w-64 shrink-0">
          <div className="card p-5 space-y-5 sticky top-20">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-text-secondary flex items-center gap-2">
                <SlidersHorizontal size={16} /> Filters
              </h2>
              {hasFilters && (
                <button
                  onClick={clearFilters}
                  className="text-xs text-accent-primary hover:underline flex items-center gap-1"
                >
                  <X size={11} /> Clear all
                </button>
              )}
            </div>

            {/* Search by city */}
            <div>
              <label
                htmlFor="filter-city"
                className="text-xs text-text-secondary mb-1.5 block uppercase tracking-wider"
              >
                City
              </label>
              <input
                type="text"
                id="filter-city"
                placeholder="Dhaka, Chattogram…"
                defaultValue={searchParams.get("city") ?? ""}
                className="form-input text-sm"
                onChange={(e) => setParam("city", e.target.value)}
              />
            </div>

            {/* Category */}
            <div>
              <label
                htmlFor="filter-category"
                className="text-xs text-text-secondary mb-1.5 block uppercase tracking-wider"
              >
                Property Type
              </label>
              <select
                className="form-input text-sm"
                value={searchParams.get("categoryId") ?? ""}
                onChange={(e) => setParam("categoryId", e.target.value)}
                id="filter-category"
              >
                <option value="">All Types</option>
                {categories?.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.name}
                  </option>
                ))}
              </select>
            </div>

            {/* Price range */}
            <div>
              <label className="text-xs text-text-secondary mb-1.5 block uppercase tracking-wider">
                Rent Range (৳/mo)
              </label>
              <div className="flex gap-2">
                <input
                  type="number"
                  id="filter-min-rent"
                  placeholder="Min"
                  defaultValue={searchParams.get("minRent") ?? ""}
                  className="form-input text-sm"
                  onChange={(e) => setParam("minRent", e.target.value)}
                />
                <input
                  type="number"
                  id="filter-max-rent"
                  placeholder="Max"
                  defaultValue={searchParams.get("maxRent") ?? ""}
                  className="form-input text-sm"
                  onChange={(e) => setParam("maxRent", e.target.value)}
                />
              </div>
            </div>

            {/* Bedrooms */}
            <div>
              <label
                htmlFor="filter-bedrooms"
                className="text-xs text-text-secondary mb-1.5 block uppercase tracking-wider"
              >
                Bedrooms
              </label>
              <select
                className="form-input text-sm"
                value={searchParams.get("bedrooms") ?? ""}
                onChange={(e) => setParam("bedrooms", e.target.value)}
                id="filter-bedrooms"
              >
                <option value="">Any</option>
                {[1, 2, 3, 4].map((n) => (
                  <option key={n} value={n}>
                    {n}+
                  </option>
                ))}
              </select>
            </div>

            {/* Availability */}
            <div>
              <label className="flex items-center justify-between text-xs text-text-secondary uppercase tracking-wider">
                Available only
                <input
                  type="checkbox"
                  id="filter-available"
                  className="accent-violet-600 h-4 w-4"
                  checked={searchParams.get("isAvailable") === "true"}
                  onChange={(e) =>
                    setParam("isAvailable", e.target.checked ? "true" : "")
                  }
                />
              </label>
            </div>
          </div>
        </aside>

        {/* ── Properties Grid ───────────────────────────────────────────── */}
        <div className="flex-1">
          {/* Sort bar */}
          <div className="flex items-center justify-between gap-3 mb-5">
            <p className="text-sm text-text-secondary">
              Showing{" "}
              <span className="text-text-primary font-medium">
                {properties.length}
              </span>{" "}
              of {meta?.total ?? 0}
            </p>
            <div className="relative">
              <ArrowUpDown size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-text-muted" />
              <select
                id="sort-options"
                value={activeSort}
                onChange={(e) => {
                  const [sortBy, sortOrder] = e.target.value.split("-");
                  const params = new URLSearchParams(searchParams.toString());
                  if (sortBy) {
                    params.set("sortBy", sortBy);
                    params.set("sortOrder", sortOrder);
                  } else {
                    params.delete("sortBy");
                    params.delete("sortOrder");
                  }
                  params.delete("page");
                  router.push(`${pathname}?${params.toString()}`);
                }}
                className="form-input text-sm pl-9 w-44"
              >
                {sortOptions.map((o) => (
                  <option key={o.value} value={o.value}>
                    {o.label}
                  </option>
                ))}
              </select>
            </div>
          </div>

          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
              {Array.from({ length: 6 }).map((_, i) => (
                <PropertyCardSkeleton key={i} />
              ))}
            </div>
          ) : properties.length === 0 ? (
            <div className="card p-12 text-center">
              <Search size={36} className="text-text-muted mx-auto mb-3" />
              <p className="text-text-primary font-medium">No properties found</p>
              <p className="text-text-muted text-sm mt-1">
                Try adjusting or clearing your filters.
              </p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-5">
                {properties.map((p) => (
                  <PropertyCard key={p.id} property={p} />
                ))}
              </div>

              {/* Pagination */}
              {meta && meta.total > meta.limit && (
                <div className="flex justify-center gap-3 mt-8">
                  {Array.from({
                    length: Math.ceil(meta.total / meta.limit),
                  }).map((_, i) => {
                    const page = i + 1;
                    return (
                      <button
                        key={page}
                        onClick={() => setParam("page", String(page))}
                        className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${
                          page === currentPage
                            ? "bg-accent-primary text-white"
                            : "bg-surface-raised text-text-secondary hover:bg-hover-bg"
                        }`}
                      >
                        {page}
                      </button>
                    );
                  })}
                </div>
              )}
            </>
          )}
        </div>
      </div>
    </div>
  );
}

export default function PropertiesPage() {
  return (
    <Suspense fallback={<div className="min-h-screen" />}>
      <PropertiesContent />
    </Suspense>
  );
}
